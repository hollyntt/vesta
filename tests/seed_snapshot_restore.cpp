...
int main() {
    ...
    foundation::vec3 velocity{100,20,0};
    unsigned cases{};
    auto expect=[&](bool expected) {
        ++cases;VESTA_CHECK(b.seed_weapon(pawn,controller,velocity,output)==expected);
    ...
    VESTA_CHECK(running_inaccuracy>.03f && running_inaccuracy<1.0f);
    ...
    setup();field(vdata,"m_nNumBullets",8);field(vdata,"m_nSpreadSeed",17);expect(true);
    VESTA_CHECK(output.num_bullets==8 && output.pattern_seed==17);
    setup();field(vdata,"m_nNumBullets",0);expect(false);
    setup();field(vdata,"m_flSpread",std::pair{NAN,.004f});expect(false);
    setup();++cases;VESTA_CHECK(b.seed_weapon(pawn,controller,output));
    VESTA_CHECK(output.velocity.x==100 && output.velocity.y==20);
    std::cout<<"seed_restore_snapshot cases="<<cases<<" required_reads="<<required_reads<<" failed=0\n";
}
