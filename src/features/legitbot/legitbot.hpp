...
namespace features::legitbot {
	...
	struct combat_frame
	{
		...
		public:
			...
			[[nodiscard]] int observe_seed_clock(
		...
		private:
			...
			// Plausible ticks x command bases x sub-tick fraction extremes.
			static constexpr std::size_t k_max_seed_samples{ 12 };

			// Every sample is a shot the click may actually produce; firing requires all.
			struct seed_shot_plan
			{
				std::array<seed_shot_sample, k_max_seed_samples> samples{};
			...
			std::uint64_t m_seed_memo_signature{};
	...
	public:
		...
		[[nodiscard]] int observe_clock(
			std::chrono::steady_clock::time_point observed_at ) noexcept
		{
			return m_controller.observe_seed_clock( observed_at );
		}

	private:
		controller_t m_controller{};
		ballistics_t m_ballistics{};
	};

	inline grenade_aim_t& grenade_aim( ) { static grenade_aim_t value{}; return value; }
	inline controller_t& controller( ) { static controller_t value{}; return value; }
	inline seed_runtime_t& seed_runtime( ) { static seed_runtime_t value{}; return value; }
