...
void write_startup_event(const char *event)
{
	...
	if (foreground)
		::GetClassNameA(foreground, foreground_class, static_cast<int>(std::size(foreground_class)));

	char line[384]{};
	const int bytes = ::sprintf_s(
	    line, "tick=%llu pid=%lu event=%s foreground=0x%p foreground_pid=%lu class=%s\r\n",
	    ::GetTickCount64(), ::GetCurrentProcessId(), event, foreground, foreground_process, foreground_class);
	if (bytes <= 0)
		return;

	const HANDLE file = ::CreateFileW(path, FILE_APPEND_DATA | GENERIC_WRITE,
...
DWORD duplicate_winlogon_token(const DWORD session_id, const DWORD desired_access, HANDLE &output)
{
	output = nullptr;
	PRIVILEGE_SET privileges{};
	privileges.PrivilegeCount = 1;
	privileges.Control = PRIVILEGE_SET_ALL_NECESSARY;
	if (!::LookupPrivilegeValueW(nullptr, L"SeTcbPrivilege", &privileges.Privilege[0].Luid))
	{
		return ::GetLastError();
	}
	...
	for (BOOL more = ::Process32FirstW(snapshot, &entry); more; more = ::Process32NextW(snapshot, &entry))
	{
		...
		if (::OpenProcessToken(process, TOKEN_QUERY | TOKEN_DUPLICATE, &token))
		{
			...
			DWORD token_session{};
			DWORD returned{};
			if (::PrivilegeCheck(token, &privileges, &has_tcb) && has_tcb &&
			    ::GetTokenInformation(token, TokenSessionId, &token_session, sizeof(token_session),
			                          &returned) &&
			    token_session == session_id)
			{
				if (::DuplicateTokenEx(token, desired_access, nullptr, SecurityImpersonation,
				                       TokenImpersonation, &output))
				{
...
DWORD create_token(HANDLE &output)
{
	...
	DWORD session_id{};
	DWORD returned{};
	if (!::GetTokenInformation(self, TokenSessionId, &session_id, sizeof(session_id), &returned))
	{
	...
	HANDLE system{};
	DWORD result = duplicate_winlogon_token(session_id, TOKEN_IMPERSONATE, system);
	...
	if (!::DuplicateTokenEx(self, TOKEN_QUERY | TOKEN_DUPLICATE | TOKEN_ASSIGN_PRIMARY | TOKEN_ADJUST_DEFAULT,
	...
	{
		BOOL value = TRUE;
		if (!::SetTokenInformation(output, TokenUIAccess, &value, sizeof(value)))
		{
			result = ::GetLastError();
			::CloseHandle(output);
			output = nullptr;
...
{
	write_startup_event(enabled() ? "uiaccess.worker_enter" : "elevated.worker_enter");
	if (enabled())
		return ERROR_SUCCESS;
	...
	if (!::CreateProcessAsUserW(token, application_name.c_str(), command_line.data(), nullptr, nullptr, FALSE,
